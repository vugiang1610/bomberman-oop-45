...
public class AILow extends AI {
    ...
    public AILow (Enemy e , Board board){
        _e = e;
        _board = board;
    }
        /**    x1 x2 x3   
     ...
     * @param Yb toa do y bom
        ...
        public int detectBombinRanger(int  Xb  ,int  Yb ){
            ...
            // chéo trên
                 if ( (Ye - Yb > 0 ) && (Ye - Yb <=radius )  ){
                     //bom ở góc trên bên phải
                     if ( (Xb - Xe) > 0 && ( Xb- Xe) <=radius ){
                         return 1;// phải trên
                     }
                     //bom ở góc trên bên trái
                     if ( (Xe - Xb) > 0 && ( Xe- Xb) <=radius ){
                         return 3; // trái trên
                     }                     
                 }
                //chéo dưới
                     if ( (Yb - Ye > 0) && ( Yb - Ye <=radius ) ){
                     //bom ở góc dưới bên phải
                     if ( (Xb - Xe) > 0 && ( Xb- Xe) <=radius ){
                         return 7;// dưới phải
                     }
                     //bom ở góc dưới bên trái
                     if ( (Xe - Xb) > 0 && ( Xe- Xb) <=radius ){
                         return 5; // dưới trái
                     }                     
                 }
	...
	public int calculateDirection() {
        ...
        // duyệt toàn bộ list bom của bảng
        for ( int i = 0 ; i < this._board.getBombs().size() ; i++) {
            ...
            // xét những quả bom trong miền đang xét
                if ( this.detectBombinRanger(Xb, Yb)!=-1 ) {
                    thread++;
                    // tùy trường hợp thì mình sẽ sét các hướng KHÔNG THỂ ĐI
                    //  chú ý 4 thay cho -1;
