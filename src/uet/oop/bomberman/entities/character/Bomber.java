...
public class Bomber extends Character {
    ...
    public void addPowerup(Item item) {
       // kiểm tra xem đã bị xóa chưa
        ...
        _powerups.add(item);
        // goi set value để thêm item vào
        item.setValues();
    }
    ...
    private void detectPlaceBomb() {
        ...
        // thời gian giữa 2 lần đăt bom liên tiếp
    ...
    protected void afterKill() {
        if (_timeAfter > 0) --_timeAfter;
            ...
            if(_bombs.size() == 0) {
				...
				if(_board.getLives() == 0) {//  hết mạng thì thua game
                    _board.endGame();
                } else {
					_board.restartLevel();
                    this._powerups.clear();
                    Game.resestAllPower();
                    // chết rồi thì reset lại các chỉ số của bomber
                }
			}
        }
    }
   ...
   */
	...
	if(xa != 0 || ya != 0)  {
		...
		// cập nhâp lại bomber đang di chuyển
                _moving = true;
                
	} else {
             // cập nhập lại là bomber chưa di chuyển
		_moving = false;              
	}
